...
| `03-tanstack-shaped-app` | Tier 1 idiomatic framework shape | Good | Standalone after PR #3; Fireline usage is visible in the example instead of hidden in shared helper code. The package script now wraps Vite with native `fireline runtime dev` and injects `FIRELINE_ENDPOINT`. | TL2 example fix | Keep browser QA evidence current as runtime-dev artifacts change. |
| `04-next-basic` | Tier 1 idiomatic framework shape | Good | Standalone after PR #3 and no longer depends on shared helper code. The package script now wraps `next dev` with native `fireline runtime dev` and maps `FIRELINE_ENDPOINT` to `NEXT_PUBLIC_FIRELINE_ENDPOINT`. | TL2 example fix | Keep fresh/reuse reviewer evidence current as runtime-dev artifacts change. |
...
| `11-server-worker-wrapper` | Tier 1 advanced server boundary reference | High-value advanced example | Auth/idempotency/tenant policy boundary is realistic but too much glue for a first app example. It is consumer-authored adapter code, not a Fireline helper. | TL1 helper/API gap if this pattern should be productized; otherwise TL2 docs | Decide whether server-side auth/idempotency wrapper should remain example-owned or become a documented helper pattern. |
...
| `18-middleware-stack` | Tier 1 middleware reference | Good specialized reference | Trace/context/budget are clean. Memory, approval gates, and subscribers are intentionally absent because backing substrate/evidence is not ready. | TL1 helper/API gap for missing middleware backings; TL2 docs for labels | Keep as conservative middleware example; add separate beads for `memory()`, approvals, or subscriber delivery before teaching them. |
...
- **Endpoint handling is now centered on `FIRELINE_ENDPOINT`:** first-read
  browser examples wrap native `fireline runtime dev` and receive the endpoint
  through child env. Advanced examples still carry more explicit daemon/stream
  recipes where their runtime or framework requires it.
...
3. **TL1 helper/API decision:** decide whether server/Worker auth,
   idempotency, and tenant-policy wrapping should remain example-owned or
   become an official helper/pattern.
4. **TL1 helper/API decision:** decide whether middleware `memory()`, approval
   gates, and webhook/Telegram subscribers are ready for runnable examples or
   should stay absent.
